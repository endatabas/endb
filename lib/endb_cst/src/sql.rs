...
peg! {
    ...
    column_alias <- ident;
    table_name <- ident / INFORMATION_SCEHMA "." ident;
    ...
    table_or_subquery <-
        table_name ( NOT ^INDEXED )?  ( FOR ^( SYSTEM TIME ( ALL / AS OF atom / FROM atom TO atom / BETWEEN atom AND atom )) )?( AS ^table_alias / !invalid_table_alias table_alias )?
