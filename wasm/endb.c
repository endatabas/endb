...
char *common_lisp_eval(char *src) {
  ...
  cl_object lisp_src = ecl_decode_from_cstring(src, -1, utf_8);
  cl_object form = cl_read_from_string(1, lisp_src);
  cl_object result = si_safe_eval(2, form, ECL_NIL);
  cl_object str =  cl_prin1_to_string(result);
  cl_fixnum res = ecl_encode_to_cstring(c_str_eval_buffer, c_str_eval_buffer_size, str, utf_8);
  if (res > c_str_eval_buffer_size) {
    ...
    res = ecl_encode_to_cstring(c_str_eval_buffer, c_str_eval_buffer_size, str, utf_8);
  }
  if (res == -1) {
    return NULL;
  }
