...
(defun endb-on-ws-message (connection message on-ws-send)
  ...
  (handler-bind ((endb/lib/server:sql-abort-query-error
                 ...
                 (com.inuoe.jzon:json-parse-error
                   (lambda (e)
                     (declare (ignore e))
                     (funcall on-ws-send (%json-rpc-error :null +json-rpc-parse-error+ "Invalid message")))))
        ...
        (handler-case
            (cond
              ((not (equal "2.0" json-rpc-version))
               (funcall on-ws-send (%json-rpc-error json-rpc-id +json-rpc-parse-error+ (format nil "Invalid version: ~A" json-rpc-version))))
              ((not (or (integerp json-rpc-id) (stringp json-rpc-id)))
               (funcall on-ws-send (%json-rpc-error json-rpc-id +json-rpc-parse-error+ (format nil "Invalid id: ~A" json-rpc-id))))
              ((not (equal "sql" json-rpc-method))
               (funcall on-ws-send (%json-rpc-error json-rpc-id +json-rpc-method-not-found+ (format nil "Invalid method: ~A" json-rpc-method))))
              ((not (fset:collection? parameters))
               (funcall on-ws-send (%json-rpc-error json-rpc-id +json-rpc-invalid-params+ (format nil "Invalid parameters: ~A" original-parameters))))
              ((not (typep manyp 'boolean))
               (funcall on-ws-send (%json-rpc-error json-rpc-id +json-rpc-invalid-params+ (format nil "Invalid many: ~A" original-manyp))))
              (t (multiple-value-bind (result result-code)
                     (endb/sql:execute-sql write-db sql parameters manyp)
                   (cond
                     ((or result (and (listp result-code)
