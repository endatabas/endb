  ...
  (destructuring-bind (ctes query &key recursive)
    ...
    (if recursive
        (let* ((new-ctes (reduce
          ...
          (multiple-value-bind (src projection)
              (ast->cl ctx query)
            (values `(let ,(loop for (cte-name) in ctes
                                 for cte = (fset:lookup new-ctes (symbol-name cte-name))
                                 collect (list (first (cte-free-vars cte)) 0))
...
(defun ast->cl (ctx ast)
  (cond
    ((eq :true ast) t)
    ((eq :false ast) nil)
    ((eq :current_date ast) `(endb/sql/expr:sql-current-date ,(fset:lookup ctx :db-sym)))
    ((eq :current_time ast) `(endb/sql/expr:sql-current-time ,(fset:lookup ctx :db-sym)))
    ((eq :current_timestamp ast) `(endb/sql/expr:sql-current-timestamp ,(fset:lookup ctx :db-sym)))
