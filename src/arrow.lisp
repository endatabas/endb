    ...
    (t (let ((row (make-array 0 :element-type 'uint8 :fill-pointer 0 :adjustable t))
         ...
         (labels ((write-blocks (start end block-size)
                    (loop with block-idx = 0
                          ...
                          do (vector-push-extend (aref x idx) row)
                             (incf block-idx)
                             (when (and (< (1+ idx) len)
                                        (= block-idx block-size))
                               (vector-push-extend 255 row)
                               (setf block-idx 0))
                          finally (when (= idx len)
                                    (let* ((pad (- block-size block-idx))
                                           (written (- block-size pad)))
                                      (dotimes (n pad)
                                        (vector-push-extend 0 row))
                                      (vector-push-extend written row))))))
           (vector-push-extend 2 row)
...
(defun arrow-struct-children (array)
  (slot-value array 'children))

(defun arrow-struct-column-value (array n column)
  (if (typep array 'dense-union-array)
      (with-slots (type-ids offsets children) array
        (arrow-struct-column-value (aref children (aref type-ids n)) (aref offsets n) column))
      (with-slots (children) array
