             ...
             (remove-if (lambda (x)
                          (trivia:match x
                            ((trivia:guard (list x _ _)
                             ...
                             t)))
                        xs))
           (build-binary-op-tree (acc xs)
             (trivia:ematch xs
               ((list* (list "BETWEEN" _ _) x (list "AND" _ _) y xs)
                (build-binary-op-tree (list :between acc (walk x) (walk y)) xs))
               ((list* (list "NOT" _ _) (list "BETWEEN" _ _) x (list "AND" _ _) y xs)
                (build-binary-op-tree (list :not (list :between acc (walk x) (walk y))) xs))
               ((list* (list "NOT" _ _) (list "IN" _ _) (list* :|subquery| x) xs)
                (build-binary-op-tree (list :not (list :in-query acc (walk (cons :|subquery| x)))) xs))
               ((list* (list "IN" _ _) (list* :|subquery| x) xs)
                (build-binary-op-tree (list :in-query acc (walk (cons :|subquery| x))) xs))
               ((list* (list "NOT" _ _) (list "IN" _ _) (list :|table_name| x) xs)
                (build-binary-op-tree (list :not (list :in-query acc (walk x))) xs))
               ((list* (list "IN" _ _) (list :|table_name| x) xs)
                (build-binary-op-tree (list :in-query acc (walk x)) xs))
               ((list* (list "NOT" _ _) (list "NULL" _ _) xs)
                (build-binary-op-tree (list :not (list :is acc :null)) xs))
               ((trivia:guard (list* (list op _ _) (list "NOT" _ _) x xs)
                              (stringp op))
                (build-binary-op-tree (list :not (list (intern op :keyword) acc (walk x))) xs))
               ((trivia:guard (list* (list "NOT" _ _) (list op _ _) x xs)
                              (stringp op))
                (build-binary-op-tree (list :not (list (intern op :keyword) acc (walk x))) xs))
               ((trivia:guard (list* (list op _ _) x xs)
                              (stringp op))
                (build-binary-op-tree (list (intern op :keyword) acc (walk x)) xs))
               (() acc)))
           (binary-op-tree (xs)
             (build-binary-op-tree (walk (first xs)) (rest xs)))
           (flatten-join (acc xs)
             (trivia:ematch xs
