...
(test isolation
  (let* ((endb/sql/expr:*sqlite-mode* t)
    ...
    (is (equalp '(#(103)) (endb/sql/db:base-table-visible-rows write-db "t1")))
    ...
    (is (equalp '(#(103)) (endb/sql/db:base-table-visible-rows db "t1")))
    (is (= 1 (endb/sql/db:base-table-size db "t1")))))
...
(test no-ddl
  (let* ((db (make-db))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(104 104) #(105 105)) result))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(1 2 2) #(2 3 :null) #(3 :null 4)) result))
      (is (equal '("a" "b" "c") columns)))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY a")
      (is (equalp '(#(1 2 2 4) #(2 3 :null :null) #(3 :null 4 :null)) result))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY a")
      (is (equalp '(#(1 2 2 4) #(2 3 :null :null) #(3 :null 4 :null) #(4 5 7 8)) result))
      (is (equal '("a" "b" "c" "d") columns)))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY a")
      (is (equalp '(#(1 2 4 4) #(2 3 :null :null) #(3 :null 4 :null) #(4 5 7 8)) result))
      (is (equal '("a" "b" "c" "d") columns)))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY a")
      (is (equalp '(#(1 2 4 4) #(2 3 :null :null) #(3 :null 4 6) #(4 5 7 8)) result))
      (is (equal '("a" "b" "c" "d") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(3 :null 4 6) #(1 2 4 4) #(:null 3 5 :null) #(4 5 7 8)) result))
      (is (equal '("a" "b" "c" "d") columns)))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY c")
      (is (equalp '(#(:null :null 4 :null 5) #(3 :null 4 6 :null) #(1 2 4 4 :null)
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t2 ORDER BY c")
      (is (equalp '(#(4 :null 4 :null 5) #(3 :null 4 6 :null) #(1 2 4 4 :null)
                    #(:null 3 5 :null :null) #(4 5 7 8 :null))
                  result))
      (is (equal '("a" "b" "c" "d" "e") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(:null 3 5) #(4 5 7)) result))
      (is (equal '("a" "b" "c") columns)))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM t3 ORDER BY c")
      (is (equalp '(#(5 :null 5) #(4 5 7)) result))
      (is (equal '("a" "b" "c") columns)))
  ...
  (let* ((db (make-db)))
    ...
    (let ((write-db (begin-write-tx db)))
      (multiple-value-bind (result columns)
        ...
        (is (equalp '(#(103 104 :null) #(:null 105 nil)) result))
        (is (equal '("a" "b" "c") columns))))
    ...
    (let ((write-db (begin-write-tx db)))
      ...
      (multiple-value-bind (result columns)
        ...
        (is (equalp '(#(:null "main" "assertion_one" "('foo;bar' < 2)"))
                    result))
      ...
      (multiple-value-bind (result columns)
        ...
        (is (equalp '(#(:null "main" "view_two" "SELECT \"b;a\\\"z\" > 4"))
                    result))
...
(test information-schema
  (let* ((db (make-db))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#("t1" "BASE TABLE")) result))
    ...
    (multiple-value-bind (result columns)
        (execute-sql write-db "SELECT * FROM information_schema.check_constraints")
      (is (equalp '(#(:null "main" "bar" "(1 = 1)")) result))
      (is (equal '("constraint_catalog" "constraint_schema" "constraint_name" "check_clause")
                 columns)))
...
(test temporal-scalars
  (let* ((db (make-db)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector (endb/arrow:parse-arrow-date-millis "2001-01-01"))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector (endb/arrow:parse-arrow-time-micros "12:01:20"))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector (endb/arrow:parse-arrow-timestamp-micros "2023-05-16T14:43:39.970062Z"))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector (endb/arrow:parse-arrow-time-micros "14:43:39.970062Z"))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "P3Y6M"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT PT12H30M5S")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "PT12H30M5S"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT INTERVAL '14:43:39.970062' HOUR TO SECOND")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "PT14H43M39.970062S"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT 2001-01-01T00:00:00 + P1Y1DT1H")
      (is (equalp (list (vector (endb/arrow:parse-arrow-timestamp-micros "2002-01-02T01:00:00"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT 2001-01-01 - P1D")
      (is (equalp (list (vector (endb/arrow:parse-arrow-date-millis "2000-12-31"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT 2001-01-01 + PT1H")
      (is (equalp (list (vector (endb/arrow:parse-arrow-timestamp-micros "2001-01-01T01:00:00"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT PT1H + 10:00:00")
      (is (equalp (list (vector (endb/arrow:parse-arrow-time-micros "11:00:00"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT P1D + P1D")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "P2D"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT P2M1D - P1M")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "P1M1D"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT 2001-01-02 - 2001-01-01")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "PT24H"))) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT 09:00:00 - 10:30:00")
      (is (equalp (list (vector (endb/arrow:parse-arrow-interval-month-day-nanos "PT1H30M"))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector nil)) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector 1)) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector :null)) result))
      (is (equal '("column1") columns)))))
...
(test temporal-current-literals
  (let* ((db (make-db))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (list (vector now)) result))
      (is (equal '("CURRENT_TIMESTAMP") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT CURRENT_TIME")
      (is (equalp (list (vector (endb/arrow:parse-arrow-time-micros "14:43:39.970062"))) result))
...
(test system-time
  (let* ((db (make-db))
    ...
    (let* ((write-db (begin-write-tx db))
      ...
      (is (equalp '(#(103 104)) (execute-sql db "SELECT * FROM t1")))
      ...
      (let* ((write-db (begin-write-tx db))
        ...
        (is (equalp '(#(101 104)) (execute-sql write-db "SELECT * FROM t1")))

        (setf db (commit-write-tx db write-db))

        (is (equalp '(#(101 104)) (execute-sql db "SELECT * FROM t1")))
        (is (equalp '(#(101 104)) (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME AS OF ?" (fset:seq system-time-as-of-update))))
        (is (equalp '(#(103 104)) (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME AS OF ?" (fset:seq system-time-as-of-insert))))
        (is (null (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME AS OF ?" (fset:seq system-time-as-of-empty))))

        (is (equalp '(#(101 104) #(103 104)) (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME BETWEEN ? AND ?" (fset:seq system-time-as-of-insert system-time-as-of-update))))
        (is (equalp '(#(103 104)) (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME BETWEEN ? AND ?" (fset:seq system-time-as-of-empty system-time-as-of-insert))))
        ...
        (is (equalp '(#(103 104)) (execute-sql db "SELECT * FROM t1 FOR SYSTEM_TIME FROM ? TO ?" (fset:seq system-time-as-of-insert system-time-as-of-update))))
        ...
        (multiple-value-bind (result columns)
          ...
          (is (equalp (list (vector 101 104 (fset:map ("start" system-time-as-of-update) ("end" endb/sql/expr:+end-of-time+)))
                            (vector 103 104 (fset:map ("start" system-time-as-of-insert) ("end" system-time-as-of-update))))
                      result))
        ...
        (multiple-value-bind (result columns)
            (execute-sql db "SELECT t1.*, t1.system_time FROM t1 FOR SYSTEM_TIME ALL")
          (is (equalp (list (vector 103 104 (fset:map ("start" system-time-as-of-insert) ("end" system-time-as-of-update))))
                      result))
          (is (equal '("a" "b" "system_time") columns)))
  ...
  (let* ((db (make-db)))
    ...
    (let ((write-db (begin-write-tx (make-db))))
      ...
      (multiple-value-bind (result columns)
        ...
        (is (equalp '(#("Street") #("Street")) result))
         ...
         (let ((db (make-directory-db :directory test-dir)))
           (unwind-protect
                (let ((write-db (begin-write-tx db)))
                  ...
                  (is (equalp '(#(103)) (execute-sql write-db "SELECT * FROM t1")))
           ...
           (let ((db (make-directory-db :directory test-dir)))
             (unwind-protect
                  (progn
                    (is (equalp '(#(103)) (execute-sql db "SELECT * FROM t1")))
                    ...
                    (let ((write-db (begin-write-tx db)))
                      ...
                      (is (equalp '(#(103) #(104)) (execute-sql write-db "SELECT * FROM t1 ORDER BY a")))
                      ...
                      (is (equalp '(#(104)) (execute-sql write-db "SELECT * FROM t1 ORDER BY a")))
           ...
           (let ((db (make-directory-db :directory test-dir)))
             (unwind-protect
                  (is (equalp '(#(104)) (execute-sql db "SELECT * FROM t1 ORDER BY a")))
               (db-close db))))
         ...
         (let ((db (make-directory-db :directory test-dir)))
           ...
           (let ((endb/storage:*tx-log-version* (1+ endb/storage:*tx-log-version*)))
             (signals-with-msg simple-error
                 "Transaction log version mismatch: 6 does not match stored: 5"
...
(test dml
  (let ((endb/sql/expr:*sqlite-mode* t)
    ...
    (multiple-value-bind (result result-code)
      ...
      (is (equalp '(#(1 2) #(1 3))
                  (endb/sql/db:base-table-visible-rows db "t1")))
      (is (equalp '(#(1 2) #(1 3)) (execute-sql db "SELECT * FROM t1 ORDER BY 2"))))
    ...
    (multiple-value-bind (result result-code)
      ...
      (is (equalp '(#(1 3) #(2 2))
                  (endb/sql/db:base-table-visible-rows db "t1")))
      (is (equalp '(#(2 2) #(1 3)) (execute-sql db "SELECT * FROM t1 ORDER BY 2"))))
...
(test dql
  (let ((endb/sql/expr:*sqlite-mode* t)
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(202)) result))
      (is (equal '("x") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT * FROM t1")
      (is (equalp '(#(103 102 100 101 104)) result))
      (is (equal '("a" "b" "c" "d" "e") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(-107692200)) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "VALUES(0,6,5.6,'jtqxx',9,5.19,'qvgba')")
      (is (equalp '(#(0 6 5.6d0 "jtqxx" 9 5.19d0 "qvgba")) result))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (%random-uuid-p (aref (first result) 0)))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#(#(210 206 33 201 210 104 64 154 177 224 73 225 32 11 250 71))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#("d2ce21c9-d268-409a-b1e0-49e1200bfa47")) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#("yv4=")) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT BASE64('yv4=')")
      (is (equalp '(#(#(202 254))) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp '(#("ac3c34dd3b4d1c52245d8e5cad42987b5027ca3d")) result))
      (is (equal '("column1") columns)))

    (multiple-value-bind (result columns)
        (execute-sql db "SELECT SHA1(2)")
      (is (equalp '(#("da4b9237bacccdf19c0760cab7aec4a8359010b0")) result))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (equalp (fset:seq 1 "foo" (fset:map ("a" (endb/arrow:parse-arrow-date-millis "2001-01-01")) ("b" :null)))
                  (interpret-sql-literal (aref (first result) 0))))
      (is (equal '("column1") columns)))
    ...
    (multiple-value-bind (result columns)
      ...
      (is (typep (aref (first result) 0) '(vector (unsigned-byte 8))))
      (is (equal '("column1") columns)))
